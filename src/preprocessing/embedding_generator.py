...
class EmbeddingGenerator:
    ...
    def __init__(
        ...
        if batch_size is None:
            self.batch_size = 256 if self.use_gpu else 64
        else:
    ...
    def setup_device(self) -> str:
        if self.use_gpu and torch.cuda.is_available():
            ...
            logger.info(f"Using CPU with batch_size={self.batch_size}")
        return self.device
    ...
    def generate_embeddings(self, texts: List[str], output_dir: str, metadata: pd.DataFrame = None) -> str:
        total_texts = len(texts)
        ...
        logger.info(f"Time: {elapsed_time/60:.2f} minutes")
        logger.info(f"Speed: {docs_per_second:.1f} docs/second")
    ...
    def _verify_embeddings(self, embeddings_path: str, expected_shape: int):
        ...
        try:
            ...
            if actual_shape == expected_shape:
                logger.info(f"Verification successful! Shape: {test_load.shape}")
                file_size_gb = os.path.getsize(embeddings_path) / (1024**3)
                logger.info(f"File size: {file_size_gb:.2f} GB")
            else:
                logger.error(f"Shape mismatch! Expected {expected_shape}, got {actual_shape}")
    ...
    def run(self, input_path: Optional[str] = None, output_dir: Optional[str] = None) -> Tuple[str, pd.DataFrame]:
        base_dir = settings.PROJECT_ROOT
        if input_path is None:
            input_path = os.path.join(base_dir, settings.DATA_PROCESSED_PATH, "merged_support_data.csv")
...
if __name__ == "__main__":
    ...
    generator = EmbeddingGenerator(batch_size=args.batch_size, use_gpu=not args.no_gpu)
    embeddings_path, _ = generator.run(input_path=args.input, output_dir=args.output)
